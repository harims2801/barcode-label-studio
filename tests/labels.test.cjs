...
test('font and colours independently change rendered output',async()=>{
  ...
  for(const [key,value] of [['shopFont',26],['designsFont',14],['taglineFont',10],['nameFont',12],['priceColor','0000FF'],['sizeColor','FF0000']]){
...
test('two-page DOCX with start offset, distinct products sharing code',async()=>{
  await init;
  const labels=Array.from({length:13},(_,i)=>({...p,size:i%2?'':'XL',price:String(500+i)}));
